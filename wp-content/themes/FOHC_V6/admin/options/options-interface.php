...
function optionsframework_tabs() {
	...
	foreach ($options as $value) {
		...
		if ($value['type'] == "tab") {
			...
			$menu .= '<li><a id="'.  esc_attr( $jquery_click_hook ) . '-tab" class="tab" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '"><i class="'. $value['icon'] .'"></i>' . esc_html( $value['name'] ) . '</a></li>';
		}
	}
...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Wrap all options
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && ( $value['type'] != "tab" ) && ( $value['type'] != "close" ) && ( $value['type'] != "open" ) && ( $value['type'] != "intro" ) ) {
		...
		case 'order':
			...
			if ( $val ) {					
				foreach ($val as $key => $option ) {
					...
					$key = explode("|",$label);
					$item = esc_html($key[0]);
					$class = esc_html($key[1]);
					
					$output .= '
					<li class="ui-state-default '. esc_html($class) .'">
						<div class="portlet-header">' . esc_attr( $item ) . '</div>
						<input id="' . esc_attr( $id ) . '" class="of-input hidden" name="' . esc_attr( $name ) . '" type="text" value="' . esc_attr( $label ) . '" />
					</li>
					';
					$i ++;
		...
		case "intro":
			...
			$output .= '' . wp_kses( $explain_value, array(
														'a' => array(
															'href' => array(),
															'title' => array()
														),
